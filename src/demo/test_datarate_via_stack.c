...
#define PING_DELAY   0  //wait before bouncing back
#else
...
//最大的payload字节长度:219
#define MAX_PAYLOAD_LEN_OF_TEST 219
...
BYTE payload[MAX_PAYLOAD_LEN_OF_TEST];
UINT16 numTimeouts;
BOOL first_packet;
			...
			case PP_STATE_SEND:
				if ((halMACTimerNowDelta(my_timer))> MSECS_TO_MACTICKS(PING_DELAY*1000)){
					aplSendMSG (APS_DSTMODE_SHORT,
						...
						1, //src EP
						&payload[0],
						MAX_PAYLOAD_LEN_OF_TEST,  //msg length
						apsGenTSN(),
...
void main (void){
	...
	payload[0]=payload[MAX_PAYLOAD_LEN_OF_TEST-1]='y';
	for(i_pload=1;i_pload<MAX_PAYLOAD_LEN_OF_TEST-1;++i_pload)
		payload[i_pload]='x';
