 ...
 * Date 2016-08-09
 * Author: li
 * A simple test for regular ping, check my children if online
...
UINT32 check_child_timer;
unsigned char ping_res;
...
void PingPong (void ) {
	...
	while(1) {
		...
		if(halMACTimerNowDelta(check_child_timer)>MSECS_TO_MACTICKS(1000)){
			check_child_timer=halGetMACTimer();
		...
		switch (ppState) {
			...
			case PP_STATE_WAIT_FOR_TX:
				...
				if (aplGetStatus() == LRWPAN_STATUS_SUCCESS) {
					...
					phy_last_tx_time = aplGetLastTxTime();
					phy_last_tx_time -= last_tx_start;
					last_tx_start = aplMacTicksToUs(phy_last_tx_time);

					printf("TX Stack latency=%uus",phy_last_tx_time);//大约是200us，满足要求，@a7190_4MHz
					conPCRLF();
				}else {
					pingTxFailures++;
					...
					my_timer= halGetMACTimer();
					ppState = PP_STATE_SEND_MAC_PING;
				}
				break;
			case PP_STATE_SEND_MAC_PING:
				ppState = PP_STATE_SEND;
				break;
		}
	}
...
LRWPAN_STATUS_ENUM usrRxPacketCallback(void) {
	...
	conPrintROMString(", RxTimeout counter: ");
