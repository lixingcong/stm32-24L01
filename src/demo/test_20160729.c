		...
		switch (ppState) {
			...
			case PP_STATE_SEND:
				if ((halMACTimerNowDelta(my_timer))> MSECS_TO_MACTICKS(PING_DELAY*1000)){
					...
					all_nodes[(IEEE_ADDRESS_ARRAY&0xff)+ALL_NODES_NUM]=A7190_ReadRSSI();

					// 目的地发往父亲
					dstADDR.saddr=aplGetParentShortAddress();
					...
					aplSendMSG (APS_DSTMODE_SHORT,
						...
						1, //src EP
						&all_nodes[0],
						ALL_NODES_NUM*2,  //msg length
						apsGenTSN(),
						FALSE);  //No APS ack requested

#endif
#ifdef LRWPAN_COORDINATOR
					if (flag_rx_count >= COORD_EMPTY_COUNT) {
						flag_rx_count = 0;
#endif
						//清空路由表
						for (node_num = 0; node_num < ALL_NODES_NUM; node_num++)
							all_nodes[node_num]=all_nodes[node_num+ALL_NODES_NUM] = 0xff;
#ifdef LRWPAN_COORDINATOR
					}
#endif
				}
...
LRWPAN_STATUS_ENUM usrRxPacketCallback(void) {
	...
	for(i=1;i<ALL_NODES_NUM;i++){
		if((*ptr)!=0xff){
			all_nodes[i]=*ptr;
			all_nodes[i+ALL_NODES_NUM]=*(ptr+ALL_NODES_NUM);
			printf("updated: node #%u 's parent is #%u, RSSI=%u\r\n",i,(*ptr),*(ptr+ALL_NODES_NUM));
		}
		ptr++;
	}
