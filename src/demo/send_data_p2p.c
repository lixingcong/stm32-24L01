 ...
 * Date 2016-07-25
 * Author: li
 * A simple test p2p send data via msstatePAN
...
// send data use laddr or saddr
#define USE_SHORT_ADDR_TO_SEND 0
UINT32 my_timer;
		...
		switch (ppState) {
			...
			case PP_STATE_SEND:
				if ((halMACTimerNowDelta(my_timer))> MSECS_TO_MACTICKS(PING_DELAY*1000)){
					...
					//format the packet
#ifdef LRWPAN_RFD
					for(i_pload=0;i_pload<20;i_pload++)
						payload[i_pload]='A'+i_pload;
					payload[20]=0;
					i_pload=21;
#endif
			...
			case PP_STATE_WAIT_FOR_TX:
				...
				if (aplGetStatus() == LRWPAN_STATUS_SUCCESS) {
					...
					printf("Data Send failed! fail code:%d, now try again.\r\n",aplGetStatus());
					my_timer= halGetMACTimer();
					ppState = PP_STATE_SEND;
				}
				break;
...
LRWPAN_STATUS_ENUM usrRxPacketCallback(void) {
	...
	dstADDR.saddr = aplGetRxSrcSADDR();
		...
		for(i=0;i<8;i++)
#ifdef LRWPAN_COORDINATOR
			dstADDR.laddr.bytes[i] = ((IEEE_ADDRESS_ARRAY_RFD>>(8*i))&0xff);
#else
			dstADDR.laddr.bytes[i] = ((IEEE_ADDRESS_ARRAY_COORD>>(8*i))&0xff);
#endif
#endif
